...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (node, history) #:: others => {
      val neighbours = neighborsWithHistory(node, history)
      val onlyNew = newNeighborsOnly(neighbours, explored + node)
      onlyNew ++ from(others ++ onlyNew, explored + node)
    }
  }
